...
export default function PalestrantesConfirmados() {
  ...
  const p = (id: string): Palestrante => ({
    ...
    nacionalidade: t.has(`palestrantes.${id}.nacionalidade`)
      ...
      : "",
    foto: FOTO_MAP[id] ?? `/palestrantes/${id}.jpeg`, // fallback com template string
  });

  // mantenha a lista de IDs exatamente como nas chaves do JSON (case-sensitive)
  const ids = [
    "robson-santos",
    "katia-viana",
    "fernando-lopez",
            ...
            >
              ...
              >
                <Image
                  ...
                  alt={`${s.nome} – ${s.cargo}`}
                  fill
                  // manter a imagem sem cortar
                  className="object-contain p-2"
                  sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
                  priority={idx < 2}
                />
                <span className="sr-only">
                  {t.has("ver_detalhes") ? t("ver_detalhes") : "Ver detalhes"}
                </span>
              </Link>
